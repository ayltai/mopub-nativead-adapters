...
function generateTestReports() {
    ...
    cp -r adapters/build/test-results/testDebugUnitTest/*.xml $CIRCLE_TEST_REPORTS && cp -r adapters/build/reports/tests/testDebugUnitTest $CIRCLE_TEST_REPORTS && cp -r adapters/build/reports/jacoco/jacocoTestReport $CIRCLE_TEST_REPORTS
}
